...
class AdventOfCodePuzzleDay03:
    ...
    # Ascii value of 'a' is 97 so we need to substract 96
    SUBSTRACTOR_LOWERCASE = 96

    # Ascii value of 'A' is 65 so we need to substract 64 minus the first 26 lower case chars a-z
    SUBSTRACTOR_UPPERCASE = 64 - 26

    GROUP_SIZE_OF_ELVES = 3

    def __init__(self, puzzle_input: list):
        self.rucksacks = puzzle_input

    def solve_puzzle_1(self) -> int:
        result = 0

        for rucksack in self.rucksacks:
            rucksack_1, rucksack_2 = self.split_item_list(rucksack)

            similarity = self._get_similarities(item1=rucksack_1, item2=rucksack_2)

            substractor = self._get_substractor(similarity=similarity)
            result += ord(similarity) - substractor
    ...
    def solve_puzzle_2(self) -> int:
        ...
        while idx < len(self.rucksacks):
            rucksack_1, rucksack_2, rucksack_3 = self.rucksacks[idx], self.rucksacks[idx+1], self.rucksacks[idx+2]

            similarity = self._get_similarities(item1=rucksack_1, item2=rucksack_2)
            similarity = self._get_similarities(item1=similarity, item2=rucksack_3)

            substractor = self._get_substractor(similarity=similarity)
            result += ord(similarity) - substractor

            idx += self.GROUP_SIZE_OF_ELVES

        return result

    def _get_substractor(self, similarity: str) -> int:
        return self.SUBSTRACTOR_LOWERCASE if similarity.islower() else self.SUBSTRACTOR_UPPERCASE

    @staticmethod
    def _get_similarities(item1: str, item2: str) -> str:
        return "".join(sorted(set(item1).intersection(set(item2))))

    @staticmethod
    def split_item_list(items: list):
        half = len(items) // 2
