...
class AdventOfCodePuzzleDay06:
    ...
    def __init__(self, puzzle_input: str):
        self.datastream = puzzle_input

    def solve_puzzle_1(self) -> int:
        return self._get_marker_position(datastream=self.datastream, chunk_size=4)

    def solve_puzzle_2(self) -> int:
        return self._get_marker_position(datastream=self.datastream, chunk_size=14)

    @staticmethod
    def _get_marker_position(datastream: str, chunk_size: int) -> int:
        for i in range(len(datastream) - chunk_size):
            if len(set(datastream[i:i + chunk_size])) == chunk_size:
                return i + chunk_size
