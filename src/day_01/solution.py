...
class AdventOfCodePuzzleDay01:
    ...
    def __init__(self, puzzle_input: list):
        self.calories = puzzle_input

    def solve_puzzle_1(self) -> int:
        return max(sum(clist) for clist in self._get_list_of_lists(self.calories))

    def solve_puzzle_2(self) -> int:
        calories_lists = [sum(clist) for clist in self._get_list_of_lists(self.calories)]
        return sum(sorted(calories_lists, reverse=True)[:3])

    @staticmethod
    def _get_list_of_lists(data: list) -> list:
        calories_lists = [list(g) for k, g in groupby(data, key=bool) if k]
        return [[int(item) for item in clist] for clist in calories_lists]
