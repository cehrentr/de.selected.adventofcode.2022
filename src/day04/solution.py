...
class AdventOfCodePuzzleDay04:
    ...
    def solve_puzzle_1(self) -> int:
        ...
        for sections in self.pairs:
            section1, section2 = self._get_resolved_sections(sections=sections)
            result += 1 if section1.issubset(section2) or section2.issubset(section1) else 0
    ...
    def solve_puzzle_2(self) -> int:
        ...
        for sections in self.pairs:
            ...
            result += 1 if intersection else 0

        return result

    @staticmethod
    def _get_resolved_sections(sections: str) -> tuple:
        section1, section2 = sections.split(",")
        section1 = {i for i in range(int(section1.split("-")[0]), int(section1.split("-")[1]) + 1)}
        section2 = {i for i in range(int(section2.split("-")[0]), int(section2.split("-")[1]) + 1)}
